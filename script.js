function hideTestFiles() {
  Array.from(
    document.querySelectorAll(
      '.file a[title$=\\.test\\.ts], .file a[title$=\\.test\\.tsx]',
    ),
  )
    .map((link) => link.closest('.file').querySelector('[aria-expanded=true]'))
    .filter(Boolean)
...
function hideTranslationFiles() {
  const files = Array.from(
    document.querySelectorAll(
      '.file[data-file-type=".json"] *[data-path*="/translations/"]',
    ),
...
function hideGraphQLFiles() {
  const files = Array.from(
    document.querySelectorAll('[data-file-type=".graphql"]'),
  );
  files.forEach((file) => file.remove());
}
function hideCSSFiles() {
  const files = Array.from(
    document.querySelectorAll(
      '[data-file-type=".css"],[data-file-type=".scss"]',
    ),
  );
  files.forEach((file) => file.remove());
}
function removeDeletedFiles() {
  const files = Array.from(
    document.querySelectorAll('[data-file-deleted=true]'),
  );
  files.forEach((file) => file.remove());
}
      ...
      const [left, right] = Array.from(
        Array.from(
          file.querySelectorAll(
            '.blob-code-deletion, .blob-code-addition, .blob-code-context, .blob-code-empty',
          ),
        ).map((aLine) => aLine.parentElement),
      );
...
function removeComments() {
  const comments = Array.from(
    document.querySelectorAll('.js-inline-comments-container'),
  );
