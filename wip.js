...
function isExport(element) {
  ...
  if (text.startsWith('export function') || text.startsWith('export const')) {
    return {hasExport: false, hasOpenExport: false};
  }
...
for (const [index, line] of Object.entries(lines)) {
  if (line.dataset.position != null) {
    ...
    const [left, right] = line.querySelectorAll(
      '.blob-code-deletion, .blob-code-addition, .blob-code-context, .blob-code-empty, .blob-code-hunk',
    );
    ...
    if (
      isEmpty(right) ||
      isImport(right).hasImport ||
      isExport(right).hasExport
    ) {
      if (importStart === -1) {
        importStart = index;
      }
